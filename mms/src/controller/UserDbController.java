...
public class UserDbController extends DbController {
	...
	public boolean createUser(User user) throws SQLException {
		...
		try {
			...
			ps.setString(6, user.getGraduation());
			ps.setString(7, user.getPassword());
			ps.setInt(8, user.getMatricNum());
			ps.setInt(9, user.getSemester());
		...
		if (user.isEmployee()) {
			...
			try {
				...
				if (employee.getEmployeeRights() == null) {
					employee.setEmployeeRights(new EmployeeRights(false, false,
							false));
				}
				...
				employee = new Employee(user.getEmail(), user.getPassword(),
						...
						user.getProfession(), user.getTitle(),
						user.getGraduation(), user.getMatricNum(),
						user.getSemester(), user.getUserRights());
			}
			...
			// Values
			query += "'" + user.getEmail() + "', "
					+ employeeRights.toEmployeeValues() + ");";
			...
			// EventRights
			ArrayList<EventRights> eventRightsList = employeeRights
					.getEventRightsList();
			...
			// ModuleRights
			ArrayList<ModuleRights> moduleRightsList = employeeRights
					.getModuleRightsList();
			...
			// SubjectRights
			ArrayList<SubjectRights> subjectRightsList = employeeRights
					.getSubjectRightsList();
			...
			ArrayList<ModuleHandbookRights> moduleHandbookRightsList = employeeRights
					.getModuleHandbookRightsList();
			if (!moduleHandbookRightsList.isEmpty()) {
				...
				for (ModuleHandbookRights mhbr : moduleHandbookRightsList) {
					...
					// Names
					query += mhbr.toValueNames() + ") VALUES('"
							+ user.getEmail() + "', ";
			...
			// StudycourseRights
			ArrayList<StudycourseRights> studycourseRightsList = employeeRights
					.getStudycourseRightsList();
	...
	/**
	 * Deletes a user, the database will delete all referring entries
	 * automatically
	 * 
	 * @param user
	...
	/**
	 * @param user
	 * @return the whole user object that belongs to the email of the passed
	 *         user
	 */
	public User getUser(User user) {
		...
		try {
			...
			if (rs.next()) {
				...
				user.setProfession(rs.getString(4));// profession
				user.setTitle(rs.getString(5)); // title
				user.setGraduation(rs.getString(6));// graduation
				user.setPassword(rs.getString(7)); // password
		...
		try {
			...
			if (rs.next()) {
				...
				System.out
						.println("[db] No user_rights found with this email");
				rs.close();
				return null;
			}
			rs.close();
		...
		Employee employee = new Employee(user.getEmail(), user.getPassword(),
				...
				user.getTitle(), user.getGraduation(), user.getMatricNum(),
				user.getSemester(), user.getUserRights());

		query = "SELECT " + employee.toEmployeeValueNames()
				+ " FROM employees " + "WHERE email='" + user.getEmail() + "';";
		...
		try {
			...
			if (rs.next()) {
				...
				System.out
						.println("[db] No employees found with this email");
				rs.close();
		...
		try {
			...
			if (rs.next()) {
				...
				System.out
						.println("[db] No employee_rights found with this email");
		...
		query = "SELECT " + eventRights.toValueNames()
				+ " FROM event_rights WHERE users_email='"
				+ employee.getEmail() + "';";
		...
		try {
			...
			if (eventRightsList.isEmpty()) {
				System.out
						.println("[db] No event_rights found with this users_email");
			} else {
		...
		query = "SELECT " + moduleRights.toValueNames()
				+ " FROM module_rights WHERE users_email='"
				+ employee.getEmail() + "';";
		...
		try {
			...
			if (moduleRightsList.isEmpty()) {
				System.out
						.println("[db] No module_rights found with this users_email");
			} else {
		...
		query = "SELECT " + subjectRights.toValueNames()
				+ " FROM subject_rights WHERE users_email='"
				+ employee.getEmail() + "';";
		...
		try {
			...
			if (subjectRightsList.isEmpty()) {
				System.out
						.println("[db] No subject_rights found with this users_email");
			} else {
		...
		query = "SELECT " + moduleHandbookRights.toValueNames()
				+ " FROM module_handbook_rights WHERE users_email='"
				+ employee.getEmail() + "';";
		...
		try {
			...
			if (moduleHandbookRightsList.isEmpty()) {
				System.out
						.println("[db] No module_handbook_rights found with this users_email");
			} else {
				// set moduleHandbookRightsList of EmployeeRights
				employeeRights
						.setModuleHandbookRightsList(moduleHandbookRightsList);
			}
			rs.close();
		} catch (SQLException e) {
			e.printStackTrace();
		...
		query = "SELECT " + studycourseRights.toValueNames()
				+ " FROM studycourse_rights WHERE users_email='"
				+ employee.getEmail() + "';";
		...
		try {
			...
			if (studycourseRightsList.isEmpty()) {
				System.out
						.println("[db] No studycourse_rights found with this users_email");
			} else {
			...
			while (rs.next()) {
				...
				if (er != null) {
					Employee employee = new Employee(user.getEmail(),
							...
							user.getLastName(), user.getProfession(),
							user.getTitle(), user.getGraduation(),
							user.getMatricNum(), user.getSemester(),
							user.getUserRights());
	...
	private EmployeeRights getEmployeeRights(User user) {
		...
		try {
			...
			if (rs.next()) {
				EmployeeRights er = new EmployeeRights(rs.getBoolean(1),
	...
	public String getHashEmail(String hash) {
		String query = "SELECT users_email FROM user_hashes " + "WHERE hash=?;";
	 ...
	 * @param firstName
	 * @param lastName
	 * @return the email that belongs to the passed firstName and lastname or
	...
	public String getUserEmail(String firstName, String lastName) {
		...
		try {
			...
			System.out.println("[db] getUserEmail: "+ps);
			...
			if (rs.next()) {
				...
				rs.close();
				System.out.println("[db] getUserEmail: no email found for this firstName and lastName");
				return null;
			}
